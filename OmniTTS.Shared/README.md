...
在 ClassIsland 启动完成后从服务容器获取服务。OmniTTS.Plugin 已注册 `IOmniTTS`，调用方通过该接口获取服务：
...
// MyClass.cs
public class MyClass(IOmniTTS tts)
{
    private IOmniTTS Tts { get; set; } = tts;
...
tts.PlayAudio("请准备开始上课。", CancellationToken.None);
```
...
使用 `option` 描述的参数生成音频，并在查询到本地缓存或生成完成后交给 ClassIsland 播放服务；任务会在播放完成后才结束。
